...
class MainFrame(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, None, -1, size=(1000, 700), title=APP_NAME)
    ...
    def InitToolBar(self):
        ...
        tb.SetToolBitmapSize((size, 2*size))
        
        tb.AddControl(wx.StaticText(tb, -1, ' Function: '))
        tb.AddControl(wx.Button(tb, -1, 'Single', size=(2*size, size)))
        tb.AddControl(wx.Button(tb, -1, 'Multi', size=(2*size, size)))
        
        tb.AddControl(wx.StaticText(tb, -1, '    Search Method: '))
        tb.AddControl(wx.Button(tb, -1, 'Random Walk', size=(3*size, size)))
        tb.AddControl(wx.Button(tb, -1, 'Hill Climbing', size=(3*size, size)))
        tb.AddControl(wx.Button(tb, -1, 'Genetic\nAlgorithm', size=(3*size, size)))
    ...
    def OnSetBest(self, x, y, z):
        ...
        if z > self.best_z:
            self.best_z = z
            self.best.SetLabel('x: {:0.3f} | y: {:0.3f}  | z: {:0.3f}'.format(x,y,z*zscale))
